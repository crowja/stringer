...
check:
	...
	do \
	  ...
	  ( $(CC)    $(CPPFLAGS) $(OTHER_INCLUDE) $(CFLAGS) $(OTHER_SOURCE) \
		-o t/a.out $$i.c stringer.c $(LDFLAGS) ) \
	  && ( t/a.out ); \
	done 
...
check-examples:
	...
	do \
	  ...
	  ( $(CC) -g $(CPPFLAGS) $(OTHER_INCLUDE) $(CFLAGS) $(OTHER_SOURCE) \
		-o ex/a.out $$i.c stringer.c $(LDFLAGS) ) \
...
echeck:
	...
	do \
	  ...
	  echo "Running test $$i ..."; \
	  ( $(CC)    $(CPPFLAGS) $(OTHER_INCLUDE) $(CFLAGS) $(OTHER_SOURCE) \
		-o t/a.out $$i.c stringer.c $(LDFLAGS_EFENCE) ) \
	  && ( LD_PRELOAD=libefence.so ./t/a.out ); \
	done 

indent: stamp
	@indent $(INDENT_FLAGS) stringer.c
	@indent $(INDENT_FLAGS) stringer.h
	@indent $(INDENT_FLAGS) t/test.c
	@indent $(INDENT_FLAGS) ex/ex_1.c

stamp:
	@bash stamper.bash stringer.c
	@bash stamper.bash stringer.h
